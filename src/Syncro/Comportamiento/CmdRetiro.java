...
Sujeto {
    ...
    public CmdRetiro(CtaBase cuenta, double monto, String descripcion) {
        this.cuenta = cuenta;
        this.monto = monto;
    ...
    public boolean ejecutar() {
        ...
        if (exito) {
            this.ejecutado = true;
            Transaccion tx = new Transaccion(Transaccion.TipoOperacion.RETIRO, this.monto, this.cuenta.getNumeroCuenta(), true, this.descripcion);
            BaseDatos.getInstance().registrarTransaccion(tx);
            BaseDatos.getInstance().actualizarSaldoCuenta(this.cuenta);
            this.notificarObservadores("RETIRO", this.cuenta.getNumeroCuenta(), this.monto, this.descripcion);
        }
        return exito;
    }
    ...
    public boolean deshacer() {
        ...
        if (exito) {
            ...
            Transaccion tx = new Transaccion(Transaccion.TipoOperacion.DEPOSITO, this.monto, this.cuenta.getNumeroCuenta(), true, "REVERSO: " + this.descripcion);
            BaseDatos.getInstance().registrarTransaccion(tx);
            BaseDatos.getInstance().actualizarSaldoCuenta(this.cuenta);
            this.notificarObservadores("REVERSO_RETIRO", this.cuenta.getNumeroCuenta(), this.monto, "Reverso de: " + this.descripcion);
        }
        return exito;
    }
    ...
    public String getDescripcion() {
        return "Retiro de $" + String.format("%,.2f", this.monto) + " de " + this.cuenta.getNumeroCuenta();
    }
